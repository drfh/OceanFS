...
void MD5_fprintf(MD5_CTX *context,FILE *of)
{
	fprintf(of,"1234567890ABCDEF");
}

void MD5_printf(MD5_CTX *context)
{
	MD5_fprintf(context,stdout);
}

#endif

void MD5_encode_str(unsigned char hash[16],unsigned char str[32])
{
	unsigned char		a;

//	MD5_memset((POINTER)hash,0,16);
//	MD5Final(hash,context);
	for(int i=0;i<16;i++)
	{
		a=hash[i]>>4;
		str[(i*2)]=(a<10?'0':'A'-10)+a;
		a=hash[i]&15;
		str[(i*2)+1]=(a<10?'0':'A'-10)+a;
	}
	str[32]='\0';
}

void MD5_hash_to_str(unsigned char hash[16],unsigned char str[32])
{
	unsigned char		a;
